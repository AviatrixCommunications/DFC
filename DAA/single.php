...
$events_link = get_field('events_calendar_link', 'option');
  ...
  <section class="single-post__header">
    <div class="wrapper">
      <p class="h1 js-fadein-up">DuPage Airport Authority News</p>
      <img class="single-post__header-logo js-fadein-up" src="<?php echo get_template_directory_uri(); ?>/img/logo-watermark.png" alt="" loading="lazy" />
    </div>
  </section>
  <section class="single-post__main">
    <div class="wrapper">
      <div class="single-post__main-inner">
        <article class="single-post__article">
          <header class="single-post__meta">
            <div class="single-post__meta-top">
              <p class="single-post__date js-fadein-up"><?php echo get_the_date(); ?></p><?php 
              $categories = get_the_category();
              if (!empty($categories)) { ?>
              <div class="single-post__categories js-fadein-up"><?php
                  foreach ($categories as $category) {
                      echo '<span class="single-post__category">' . esc_html($category->name) . '</span>';
                  } ?>
              </div><?php
              } ?>
            </div>
            <h1 class="single-post__title h2 js-fadein-up"><?php the_title(); ?></h1>
          </header><?php 
          if ( has_post_thumbnail() ) { ?>
          <div class="single-post__featured-image js-fadein-up">
              <?php the_post_thumbnail('hero'); ?>
          </div>
          <?php } ?>
          <div class="single__post-content editor-content">
              <?php the_content(); ?>
          </div>
          <a class="single-post__back js-fadein-up" href="/news-events">
            <svg xmlns="http://www.w3.org/2000/svg" width="8" height="14" viewBox="0 0 8 14" fill="none">
              <g clip-path="url(#clip0_12387_3984)">
                <path fill-rule="evenodd" clip-rule="evenodd" d="M0.843063 6.28898L6.50006 0.631985L7.91406 2.04598L2.96406 6.99599L7.91406 11.946L6.50006 13.36L0.843063 7.70299C0.655592 7.51546 0.550276 7.26115 0.550276 6.99599C0.550276 6.73082 0.655592 6.47651 0.843063 6.28898Z" fill="black"/>
              </g>
              <defs>
                <clipPath id="clip0_12387_3984">
                  <rect width="8" height="14" fill="white" transform="matrix(-1 0 0 -1 8 14)"/>
                </clipPath>
              </defs>
            </svg>
            <span>Back to the Newsroom</span>
          </a>
        </article>
        <aside class="single-post__sidebar"><?php 
          if ( $media_email || $media_phone ) { ?>
          <div class="single-post__sidebar-box single-post__sidebar-box--media js-fadein-up">
            <h3>
          ...
          if ( $events_link ) { ?>
          <div class="single-post__sidebar-box single-post__sidebar-box--events js-fadein-up">
            <h3>
              <a href="<?php echo $events_link; ?>" target="_blank">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div class="single-post__sidebar-box single-post__sidebar-box--social js-fadein-up">
            <h3>Follow Us on Social Media</h3>
            <nav class="single-post__sidebar-social nav--social" aria-label="Social media links">
              <ul>
                <?php
                $social_links = [
